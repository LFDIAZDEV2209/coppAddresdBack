...
/// <summary>
/// Seed de la plantilla por defecto del programa de 83 días / 12 semanas
/// (<c>program-coppaddresd-83-days</c>, el programa inicial y principal de todos
/// los pacientes): una fila en <c>app.program_templates</c> y 42 filas
...
/// <c>program-coppaddresd-83-days</c>). El <c>media_id</c> de las filas queda NULL a propósito:
...
public sealed class ProgramProgressSeeder(
    ...
    ILogger<ProgramProgressSeeder> logger
) : IHostedService
{
    private readonly string _defaultTemplateCode =
        // El programa inicial y principal de TODOS los pacientes es el de 83 días
        // (12 semanas). El fallback anterior ("default-83w") sembraba una plantilla
        // de 83 semanas por una confusión al crearla — corregido (fix pedido por
        // Luis, pre-FASE 4).
    ...
    private async Task SeedAsync(CancellationToken ct)
    {
        ...
        await SeedProgramTemplateAsync(
            "program-coppaddresd-83-days",
            "Programa COPP-ADRESD (83 días / 12 semanas)",
            "Plantilla del programa de 83 días (12 semanas) para pacientes Copp Adresd.",
            12,
            ct
        );
        if (_defaultTemplateCode != "program-coppaddresd-83-days")
        {
            await SeedProgramTemplateAsync(
                ...
                "Programa 83 semanas",
                "Plantilla integral por defecto del programa de 83 semanas (auto-sembrada).",
                83,
                ct
            );
        }
    ...
    )
    {
        var templateId = await WithContext(
            db =>
                db.ProgramTemplates.Where(x => x.Code == code)
                    .Select(x => (Guid?)x.Id)
        ...
        if (templateId is not null)
        {
            var dayCount = await WithContext(
                db => db.WeeklyDayTemplates.CountAsync(x => x.TemplateId == templateId, ct),
                ct
            );
    ...
    private async Task SeedHealthScoreWeightsAsync(CancellationToken ct)
    {
        ...
        {
            (
                ScoreDimension.adherence,
                0.30m,
                "Adherencia al programa (check-ins y congelamientos)"
            ),
        ...
        foreach (var (dimension, weight, description) in defaultWeights)
        {
            var exists = await WithContext(
                db => db.HealthScoreWeights.AnyAsync(x => x.Dimension == dimension, ct),
                ct
            );
            ...
            await WithContext(
                async db =>
                {
                    db.HealthScoreWeights.Add(
                        new HealthScoreWeight
                        {
                            Dimension = dimension,
                            Weight = weight,
                            Description = description,
    ...
    private async Task SeedXpRulesAsync(CancellationToken ct)
    {
        var defaultRules = new[]
        {
            ...
            new XpRuleSeed(
                XpRuleCodes.TaskEmocional,
                "Tarea evaluación emocional",
                "adherence",
                null,
                1,
                7
            ...
            new XpRuleSeed(
                ...
                "Hidratación registrada (XP granular)",
                "nutrition",
                5,
                1,
                7
            ...
            new XpRuleSeed(
                ...
                "Adherencia nutricional semanal ≥ 85%",
                "nutrition",
                75,
                1,
                1
            ...
            new XpRuleSeed(
                ...
                "Recuperación nutricional (+20pp vs período anterior)",
                "nutrition",
                50,
                1,
                1
            ...
            new XpRuleSeed(
                XpRuleCodes.NbStreak7,
                "Hito racha nutracéutico 7 días",
                "nutriobiotic",
                50,
                1,
                1
            ...
            new XpRuleSeed(
                XpRuleCodes.NbStreak14,
                "Hito racha nutracéutico 14 días",
                "nutriobiotic",
                100,
                1,
                1
            ...
            new XpRuleSeed(
                XpRuleCodes.NbStreak30,
                "Hito racha nutracéutico 30 días",
                "nutriobiotic",
                250,
                1,
                1
            ...
            new XpRuleSeed(
                XpRuleCodes.NbStreak60,
                "Hito racha nutracéutico 60 días",
                "nutriobiotic",
                500,
                1,
                1
            ...
            new XpRuleSeed(
                XpRuleCodes.NbStreak90,
                "Hito racha nutracéutico 90 días",
                "nutriobiotic",
                1000,
                1,
                1
            ...
            new XpRuleSeed(
                ...
                "Evaluación de debilidad (intervención creada)",
                "intervention",
                20,
                1,
                1
            ...
            new XpRuleSeed(
                ...
                "Teleconsulta asistida (validación clínica)",
                "intervention",
                100,
                null,
                null,
            ...
            new XpRuleSeed(
                ...
                "Intervención completada (validación clínica)",
                "intervention",
                200,
                null,
                null,
            ...
            new XpRuleSeed(
                ...
                "Misión de recuperación aceptada",
                "intervention",
                50,
                1,
                7
        ...
        {
            var exists = await WithContext(
                db => db.XpRules.AnyAsync(x => x.Code == seed.Code, ct),
                ct
            );
            ...
            logger.LogInformation(
                ...
                seed.Code,
                seed.Category,
                seed.BaseXp?.ToString() ?? "template",
                seed.RequiresValidation
            );
        }
    }
    ...
    private async Task SeedNutritionHabitTemplatesAsync(CancellationToken ct)
    {
        ...
        {
            var exists = await WithContext(
                db => db.HabitTemplates.AnyAsync(x => x.Code == code, ct),
                ct
            );
