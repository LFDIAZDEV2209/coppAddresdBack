...
public sealed class OrganizationRepository(AppDbContext dbContext) : IOrganizationRepository
{
    ...
    public async Task UpdateOrganizationAsync(Organization organization, CancellationToken ct = default)
    {
        ...
        await dbContext.Organizations
            ...
            .ExecuteUpdateAsync(setters => setters
                ...
                .SetProperty(x => x.UpdatedAt, organization.UpdatedAt), ct);
    }
    ...
    public async Task UpdateLocationAsync(Location location, CancellationToken ct = default)
    {
        await dbContext.Locations
            ...
            .ExecuteUpdateAsync(setters => setters
                ...
                .SetProperty(x => x.UpdatedAt, location.UpdatedAt), ct);
    }

    public async Task<IReadOnlyList<ProfessionalType>> ListProfessionalTypesAsync(CancellationToken ct = default)
        => await dbContext.ProfessionalTypes
            .AsNoTracking()
            .Include(x => x.Specialties)
            .OrderBy(x => x.SortOrder)
            .ToListAsync(ct);

    public async Task<IReadOnlyList<Specialty>> ListSpecialtiesAsync(CancellationToken ct = default)
    ...
    public async Task<IReadOnlyList<Specialty>> ListSpecialtiesByProfessionalTypeAsync(
        ...
        => await dbContext.ProfessionalTypeSpecialties
            .AsNoTracking()
    ...
    public async Task<ProfessionalType?> GetProfessionalTypeByIdAsync(Guid id, CancellationToken ct = default)
        => await dbContext.ProfessionalTypes
            .AsNoTracking()
            .FirstOrDefaultAsync(x => x.Id == id, ct);
