...
/// <summary>
/// Abstracción única de caché distribuida del Auth Service (contrato idéntico
/// al <c>ICacheService</c> del backend ERP: el servicio es standalone y no
...
public interface ICacheService
{
    ...
    /// <summary>Invalida una clave (llamar en el mismo flujo de la mutación). Best-effort.</summary>
    Task RemoveAsync(string key, CancellationToken ct = default);

    /// <summary>
    /// Cache-aside: devuelve el valor cacheado y, en miss, lo construye con
    /// <paramref name="factory"/> y lo guarda con el TTL dado. Sin lock
    /// distribuido: la reconstrucción es idempotente y barata.
    /// </summary>
