...
public interface IRoleService
{
    ...
    Task<(bool Success, string? Error, RoleResponse? Role)> CreateAsync(
    ...
    Task<(bool Success, string? Error)> AssignToUserAsync(
        Guid userId,
        Guid roleId,
        CancellationToken ct = default
    );
    Task<(bool Success, string? Error)> RemoveFromUserAsync(
        Guid userId,
        Guid roleId,
        CancellationToken ct = default
    );
