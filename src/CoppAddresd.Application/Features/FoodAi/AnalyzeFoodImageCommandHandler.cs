    ...
    : IRequestHandler<AnalyzeFoodImageCommand, AnalyzeFoodImageResult>
{
    ...
    )
    {
        ...
        var imageKey = await _imageStorage.SaveImageAsync(
            analysisId,
            request.FileName,
            forStorage,
            ct
        ...
        {
            // Porciones degeneradas (0 g o menos) se normalizan a null para que
            // ningún cliente muestre "0 g" como porción válida.
            var normalized = food.Portion?.EstimatedGrams is null or <= 0
                ? food with
                {
                    Portion = null,
                }
                : food;
            var nutritionResult = await ResolveNutritionAsync(normalized, ct);
            foods.Add(normalized with { NutritionResult = nutritionResult });
        }

        var summary = ComputeSummary(foods);
        var intake = MapIntake(summary.Summary);
        ...
        var failures = foods
            .Where(f =>
                f.NutritionResult is not null && f.NutritionResult.NutritionStatus != "available"
            )
        ...
        _logger.LogInformation(
            ...
            statuses.GetValueOrDefault("portion_unavailable")
    ...
    )
    {
        try
        {
            var analysis = new FoodAnalysis
            {
                ...
                Source = foods
                    .FirstOrDefault(f => f.NutritionResult?.Source is not null)
                    ?.NutritionResult?.Source,
                SourceVersion = foods
                    .FirstOrDefault(f => f.NutritionResult?.SourceVersion is not null)
                    ?.NutritionResult?.SourceVersion,
            };
            ...
            for (var index = 0; index < foods.Count; index++)
            {
                ...
                string? maskKey = null;
                if (
                    food.Segmentation is not null
                    && !string.IsNullOrWhiteSpace(food.Segmentation.Mask)
                )
                {
                    try
                    {
                        ...
                        maskKey = await _imageStorage.SaveMaskAsync(
                            analysisId,
                            index,
                            maskStream,
                            ct
                    ...
                    catch (Exception ex)
                    {
                        ...
                        _logger.LogWarning(
                            ex,
                            "No se pudo guardar la máscara del item {Index}",
                            index
                        );
                    }
                }
            ...
            _logger.LogInformation(
                "Análisis persistido: AnalysisId={AnalysisId}, Items={ItemCount}",
                analysisId,
                foods.Count
            );
        }
    ...
    )
    {
        ...
        return _nutritionCalculator.Calculate(
            per100g,
            food.Portion.EstimatedGrams,
            food.Portion.MinGrams,
            food.Portion.MaxGrams
        );
    }

    /// <summary>
    /// Totales del análisis en forma de ingesta (contrato de integraciones).
    /// Null sin resumen — el cliente distingue "sin datos" de "ceros".
    /// </summary>
    private static MealIntakeDto? MapIntake(NutritionValueDto? summary) =>
        summary is null
            ? null
            : new MealIntakeDto(
                summary.Calories,
    ...
    )
    {
        ...
        var available = foods
            .Where(f =>
                f.NutritionResult?.NutritionStatus == "available"
                && f.NutritionResult.Nutrition is not null
            )
            .ToList();
        ...
        decimal Sum(Func<NutritionValueDto, decimal> selector) =>
            Math.Round(
                available.Sum(f => selector(f.NutritionResult!.Nutrition!)),
                2,
        ...
        NutritionValueDto? SumRange(
            Func<NutritionValueDto, decimal> selector,
            Func<FoodNutritionResult, NutritionValueDto> pick
        )
        {
        ...
        var summaryRange =
            min is not null && max is not null ? new NutritionRangeDto(min, max) : null;
