...
public sealed class FakeReferenceDataService : ITelemedicineReferenceDataService
{
    ...
    public Task<ProfessionalRefDto?> GetProfessionalAsync(Guid professionalId, CancellationToken ct = default)
    ...
    public Task<SpecialtyRefDto?> GetSpecialtyAsync(Guid specialtyId, CancellationToken ct = default)
        => Task.FromResult(Specialties.GetValueOrDefault(specialtyId));

    public Task<LocationRefDto?> GetLocationAsync(Guid locationId, CancellationToken ct = default)
        => Task.FromResult(Locations.GetValueOrDefault(locationId));

    public Task<ProfessionalRefDto?> GetProfessionalByUserIdAsync(Guid userId, CancellationToken ct = default)
        => Task.FromResult(
            UserToProfessional.TryGetValue(userId, out var id)
                ? Professionals.GetValueOrDefault(id)
                : null);

    public Task<PatientRefDto?> GetPatientByUserIdAsync(Guid userId, CancellationToken ct = default)
        => Task.FromResult(
            UserToPatient.TryGetValue(userId, out var id)
                ? Patients.GetValueOrDefault(id)
                : null);
}
...
public sealed class FakeSettingsProvider : ITelemedicineSettingsProvider
{
    ...
    public Task<TelemedicineSettings> GetSettingsAsync(Guid organizationId, Guid? clinicId, CancellationToken ct = default)
    {
        Calls++;
...
public sealed class FakeVideoProvider : IVideoProvider
{
    ...
    public Task<IReadOnlyList<ParticipantInfo>> GetParticipantsAsync(string providerRoomSid, CancellationToken ct)
        => Task.FromResult<IReadOnlyList<ParticipantInfo>>([]);

    public Task<bool> ValidateWebhookSignatureAsync(WebhookValidationRequest request, CancellationToken ct)
...
public sealed class FakeAppointmentRepository : IAppointmentRepository
{
    ...
    public Task<TelemedicineAppointment?> GetForUpdateAsync(Guid id, CancellationToken ct = default)
        => Task.FromResult(Items.FirstOrDefault(a => a.Id == id));

    public Task<TelemedicineAppointment> AddAsync(TelemedicineAppointment appointment, CancellationToken ct = default)
    {
    ...
    public Task<bool> HasActiveOverlapAsync(Guid professionalId, DateTimeOffset start, DateTimeOffset end, Guid? excludeAppointmentId = null, CancellationToken ct = default)
    ...
    public Task<IReadOnlyList<TelemedicineAppointment>> ListByProfessionalAsync(Guid professionalId, DateTimeOffset from, DateTimeOffset to, CancellationToken ct = default)
        => Task.FromResult<IReadOnlyList<TelemedicineAppointment>>(Items
            .Where(a => a.ProfessionalId == professionalId && a.ScheduledStart >= from && a.ScheduledStart < to)
            .OrderBy(a => a.ScheduledStart)
            .ToList());

    public Task<IReadOnlyList<TelemedicineAppointment>> ListByPatientAsync(Guid patientId, CancellationToken ct = default)
        => Task.FromResult<IReadOnlyList<TelemedicineAppointment>>(Items
        ...
        DateTimeOffset? from, DateTimeOffset? to, int page, int pageSize, CancellationToken ct = default)
    {
        ...
        if (locationId is not null) query = query.Where(a => a.LocationId == locationId);
        if (status is not null) query = query.Where(a => a.Status == status);
        if (from is not null) query = query.Where(a => a.ScheduledStart >= from);
        if (to is not null) query = query.Where(a => a.ScheduledStart < to);

        var list = query.OrderByDescending(a => a.ScheduledStart).ToList();
        return Task.FromResult((list.Skip((page - 1) * pageSize).Take(pageSize).ToList() as IReadOnlyList<TelemedicineAppointment>, list.Count));
    }
...
public sealed class FakeRequestRepository : IRequestRepository
{
    ...
    public Task<TelemedicineRequest> AddAsync(TelemedicineRequest request, CancellationToken ct = default)
    {
        Items.Add(request);
    ...
    public Task SetStatusAsync(Guid requestId, AppointmentRequestStatus status, CancellationToken ct = default)
    {
    ...
    public Task<IReadOnlyList<TelemedicineRequest>> ListByPatientAsync(Guid patientId, CancellationToken ct = default)
        => Task.FromResult<IReadOnlyList<TelemedicineRequest>>(Items
            .Where(r => r.PatientId == patientId).OrderByDescending(r => r.CreatedAt).ToList());

    public Task<(IReadOnlyList<TelemedicineRequest> Items, int Total)> ListByOrganizationAsync(
        Guid organizationId, AppointmentRequestStatus? status, int page, int pageSize, CancellationToken ct = default)
    {
        var query = Items.Where(r => r.OrganizationId == organizationId);
        if (status is not null) query = query.Where(r => r.Status == status);
        var list = query.ToList();
        return Task.FromResult((list.Skip((page - 1) * pageSize).Take(pageSize).ToList() as IReadOnlyList<TelemedicineRequest>, list.Count));
    }

    public Task<(IReadOnlyList<TelemedicineRequest> Items, int Total)> ListAdminAsync(
        AppointmentRequestStatus? status, Guid? professionalId, Guid? patientId,
        DateTimeOffset? from, DateTimeOffset? to, int page, int pageSize, CancellationToken ct = default)
    {
        var query = Items.AsEnumerable();
        if (status is not null) query = query.Where(r => r.Status == status);
        if (professionalId is not null) query = query.Where(r => r.ProfessionalId == professionalId);
        if (patientId is not null) query = query.Where(r => r.PatientId == patientId);
    ...
    public Task<int> CountByStatusAsync(AppointmentRequestStatus status, CancellationToken ct = default)
...
public sealed class FakeRoomRepository : IRoomRepository
{
    ...
    public Task<VirtualRoom?> GetByAppointmentIdAsync(Guid appointmentId, bool includeSessions = false, CancellationToken ct = default)
        => Task.FromResult(Rooms.FirstOrDefault(r => r.AppointmentId == appointmentId));

    public Task<VirtualRoom?> GetByProviderRoomSidAsync(string providerRoomSid, bool includeSessions = false, CancellationToken ct = default)
        => Task.FromResult(Rooms.FirstOrDefault(r => r.ProviderRoomSid == providerRoomSid));

    public Task<VirtualRoom?> GetForUpdateAsync(Guid appointmentId, CancellationToken ct = default)
        => Task.FromResult(Rooms.FirstOrDefault(r => r.AppointmentId == appointmentId));

    public Task<VirtualRoom?> GetForUpdateByProviderRoomSidAsync(string providerRoomSid, CancellationToken ct = default)
        => Task.FromResult(Rooms.FirstOrDefault(r => r.ProviderRoomSid == providerRoomSid));
    ...
    public Task UpdateAsync(VirtualRoom room, CancellationToken ct = default)
        => Task.CompletedTask;

    public Task AddWebhookEventAsync(TelemedicineWebhookEvent webhookEvent, CancellationToken ct = default)
    {
        if (WebhookEvents.Any(e =>
    ...
    public Task<(IReadOnlyList<TelemedicineSession> Items, int Total)> ListSessionsAsync(
        Guid? appointmentId, DateTimeOffset? from, DateTimeOffset? to, int page, int pageSize, CancellationToken ct = default)
    {
        var query = Sessions.AsEnumerable();
        if (appointmentId is not null) query = query.Where(s => s.AppointmentId == appointmentId);
        var list = query.OrderByDescending(s => s.StartedAt ?? s.CreatedAt).ToList();
        return Task.FromResult((list.Skip((page - 1) * pageSize).Take(pageSize).ToList() as IReadOnlyList<TelemedicineSession>, list.Count));
    }
...
public sealed class FakeAlertRepository : IAlertRepository
{
    ...
    public Task AddRangeAsync(IReadOnlyList<TelemedicineAlert> alerts, CancellationToken ct = default)
    {
        Items.AddRange(alerts);
        return Task.CompletedTask;
    }

    public Task<(IReadOnlyList<TelemedicineAlert> Items, int Total)> ListForUserAsync(Guid recipientUserId, bool unreadOnly, int page, int pageSize, CancellationToken ct = default)
    {
        var query = Items.Where(a => a.RecipientUserId == recipientUserId);
        if (unreadOnly) query = query.Where(a => a.ReadAt == null);
        var list = query.OrderByDescending(a => a.CreatedAt).ToList();
        return Task.FromResult((list.Skip((page - 1) * pageSize).Take(pageSize).ToList() as IReadOnlyList<TelemedicineAlert>, list.Count));
    }
    ...
    public Task<(IReadOnlyList<TelemedicineAlert> Items, int Total)> ListAllAsync(bool unreadOnly, int page, int pageSize, CancellationToken ct = default)
    {
        var query = Items.AsEnumerable();
        if (unreadOnly) query = query.Where(a => a.ReadAt == null);
        var list = query.OrderByDescending(a => a.CreatedAt).ToList();
    ...
    public Task<int> MarkAllReadGlobalAsync(CancellationToken ct = default)
    {
        ...
        foreach (var a in Items.Where(a => a.ReadAt == null)) a.ReadAt = DateTimeOffset.UtcNow;
    ...
    public Task<bool> MarkReadAsync(Guid alertId, Guid recipientUserId, CancellationToken ct = default)
    {
    ...
    {
        var targets = Items.Where(a => a.RecipientUserId == recipientUserId && a.ReadAt == null).ToList();
        foreach (var a in targets) a.ReadAt = DateTimeOffset.UtcNow;
...
public sealed class FakeEncounterRepository : IEncounterRepository
{
    ...
    public Task<ClinicalEncounter?> GetByAppointmentIdAsync(Guid appointmentId, CancellationToken ct = default)
        => Task.FromResult(Items.FirstOrDefault(e => e.AppointmentId == appointmentId));

    public Task<ClinicalEncounter?> GetForUpdateByAppointmentIdAsync(Guid appointmentId, CancellationToken ct = default)
        => Task.FromResult(Items.FirstOrDefault(e => e.AppointmentId == appointmentId));

    public Task<ClinicalEncounter> AddAsync(ClinicalEncounter encounter, CancellationToken ct = default)
    {
...
{
    public static IOptions<TelemedicineOptions> Create(string webhookUrl = "https://x/api/v1/telemedicine/webhooks/twilio")
