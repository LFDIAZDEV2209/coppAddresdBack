...
/// <summary>
/// Definición de la colección de tests de integración. Todos los tests del
/// microservicio comparten UNA base de datos aislada por corrida
/// (<see cref="TelemedicineTestDatabase"/>): se crea, se migra (schema <c>tele.</c>)
/// y se elimina al terminar. No toca la base de desarrollo.
/// </summary>
[CollectionDefinition(Name)]
public sealed class TelemedicineTestCollection : ICollectionFixture<TelemedicineTestDatabase>
{
    public const string Name = "telemedicine-integration";
}

/// <summary>
/// Base de datos de prueba aislada para el microservicio de Telemedicina.
/// Requiere la variable <c>COP_TEST_DB_CONNECTION</c> apuntando a la instancia de
/// PostgreSQL (misma convención que los tests de integración del backend). El
/// usuario debe poder crear bases (CREATEDB). Si la variable no está definida,
/// los tests se omiten.
/// </summary>
public sealed class TelemedicineTestDatabase : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        DatabaseName = $"coppaddresd_tele_test_{Guid.NewGuid():N}";
        ...
        await using var db = new TelemedicineDbContext(options);
