...
public class PatientScopingTests
{
    ...
    private readonly ILogger<CreatePatientCommandHandler> _createLogger =
        Substitute.For<ILogger<CreatePatientCommandHandler>>();
    private readonly ILogger<UpdatePatientCommandHandler> _updateLogger =
        Substitute.For<ILogger<UpdatePatientCommandHandler>>();
    ...
    public async Task Create_ConClinicaActiva_AsignaClinicaYActores()
    {
        ...
        var result = await handler.Handle(new CreatePatientCommand(
            ...
            Medications: null,
            Allergies: null,
            VitalSigns: null), CancellationToken.None);
    ...
    public async Task Update_RegistraUpdatedBy()
    {
        ...
        var result = await handler.Handle(new UpdatePatientCommand(
            Id: existing.Id,
    ...
    public async Task Delete_UsaSoftDeleteYRegistraActor()
    {
        ...
        var result = await handler.Handle(new DeletePatientCommand(existing.Id, deletedBy), CancellationToken.None);
    ...
    public async Task List_PasaClinicIdAlRepositorio()
    {
        ...
        var result = await handler.Handle(new ListPatientsQuery(ClinicId: clinicId), CancellationToken.None);
