...
| Liga del paciente (LEAGUE v1): cohorte por estado (o nacional con &lt;10 opt-in)                                                                 | `league:<stateCode\|ALL>:v1`                      | 5 min            | `Remove` de `league:{estado}` + `league:ALL` del paciente en su propio cambio de preferencias (revocación del opt-in inmediata); cambios de OTROS pacientes → TTL | Cohortes compartidos por todos los pacientes del mismo alcance; sin datos por-petición (me/isMe se resuelven en el handler). El cohorte nacional puede existir bajo varias claves (`league:ALL:v1` + `league:{ST}:v1` de estados chicos) con computedAt independientes — inofensivo, por diseño (cada clave expira sola) |
| Historial de puntajes del paciente (scores-history, pestaña Evolución)                                                                         | `scores-history:<patientId>:v1`                  | 5 min            | `Remove` en la re-inscripción del paciente (purga de la corrida anterior, SPEC §13.7.3 — historia por programa); cambios semanales → TTL | Serie completa por paciente (nunca compartida entre pacientes; el scoping lo garantiza la propia clave); el recorte a las últimas N semanas se aplica por request |
| Historial de métricas clínicas del paciente (metrics-history, Home móvil)                                                                      | `metrics-history:<patientId>:v1`                 | 5 min            | Sin invalidación explícita: el TTL absorbe el lag de completaciones de signos vitales (≤ 5 min, mismo tradeoff que scores-history) | CONTEXTO COMPLETO cacheado (serie de todas las métricas activas sobre la ventana máxima 365d + talla + targets, sin codes/days en la clave); el recorte por-request (whitelist → 400, clamp de días, re-filtro de ventana, fallback IMC) se aplica después del caché y NUNCA se cachea (precedente scores-history) |
