...
| AC-34 | Patient logs hydration via `POST /program/nutrition/log { mealCode: 'agua' }`. | PRIMER log del día: 1 `app.habit_checks` row + 1 `xp_ledger` row `NUTRITION_HYDRATION` +5, tope 1/día (regla `NUTRITION_HYDRATION`). **Repetir agua el mismo día NO es un duplicado** (hidratación acumulable, tarjeta de 8 vasos): responde `200` con `xpAwarded = 0`, sin segundo habit_check ni XP; la fila de `app.nutrition_intake_logs` anclada al habit_check existente actualiza su `waterMl` SOLO si el payload trae un total MAYOR (p. ej. 250 → 500 → 750; menor/igual o sin `waterMl` → no-op idempotente; sin fila de intake — anomalía pre-SPEC — → se crea anclada sin XP). Las comidas conservan `409 HABIT_ALREADY_LOGGED` en el duplicado (AC-33). |
...
| Duplicado (comidas des/alm/mer/cen) | `409 HABIT_ALREADY_LOGGED` — la comida de esa fecha ya está registrada (semántica "ya registrado" del módulo, precedente `REVIEW_ALREADY_DECIDED`; first-write-wins, sin update). |
| Repetición (agua) | **NO es duplicado** — la hidratación es acumulable en el día: responde `200` con `xpAwarded = 0` y actualiza el total acumulado `waterMl` de la fila de `nutrition_intake_logs` anclada al habit_check existente (solo sube: payload MAYOR que el actual → update, y el `source` si el payload trae uno no vacío; menor/igual o sin `waterMl` → no-op idempotente; sin fila de intake — anomalía pre-SPEC — → se crea anclada, SIN XP). `NUTRITION_HYDRATION` se otorga exactamente 1/día, en el primer log. |
| La XP nunca se duplica | El único de `habit_checks` + el dedupe parcial `('habit_log', habit_check.id, reason)` son el backstop de carrera (comidas y primer log de agua). |
