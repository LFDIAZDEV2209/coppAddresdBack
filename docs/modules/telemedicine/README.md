...
- **Barrido de sesiones estancadas**: `StaleSessionSweepHostedService` (cada 5 min, delay inicial 1 min) cierra citas `InProgress` cuyo fin programado ya pasó más la gracia de `RoomCloseAfterMinutes` efectiva (settings por org/clínica): `NoShow` si el paciente nunca ingresó (`patient_joined_at` null), `Completed` si ingresó. La sala del proveedor se completa best-effort y la sesión activa se cierra con `end_reason = stale-sweep`.
