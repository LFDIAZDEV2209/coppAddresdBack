...
| 2026-09-04 | Pre-agregación CQRS Analítica (Fase 1) | Canales `System.Threading.Channels` + HostedServices con atomic upsert PostgreSQL en background (0ms write latency overhead). Detalle: [`analytics-cqrs-preaggregation.md`](analytics-cqrs-preaggregation.md). |
...
| 2026-09-04 | Pre-agregación CQRS Infraestructura (Patients, ProgramProgress, HealthTests, Telemedicine, Community) | Rollups, colas y processors transplantados; wiring de write-paths pendiente (iteración siguiente). |
